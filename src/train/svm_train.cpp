...
void SvmTrain::train() {
  ...
  //为了学习如何训练将其注释，后面需要将注释去掉
  auto train_data = tdata();                                   //读取数据，计算特征，读取label，返回值给train_data

  fprintf(stdout, ">> Training SVM model, please wait...\n");
  long start = utils::getTimestamp();   //计算时间？？？
  ...
  svm_->save(svm_xml_);                       //生成得到的tmp文件夹下面的svm.xml就是训练好的模型

  fprintf(stdout, ">> Your SVM Model was saved to %s\n", svm_xml_);
  fprintf(stdout, ">> Testing...\n");

  this->test();   //测试训练得分及losss等
...
void SvmTrain::prepare() {
  ...
  auto has_file_train_list = utils::getFiles(buffer);    //tmp/has/train目录下存放用于训练数据
  std::random_shuffle(has_file_train_list.begin(), has_file_train_list.end());

  sprintf(buffer, "%s/has/test", plates_folder_);     //tmp/has/test目录下存放用于测试数据
...
cv::Ptr<cv::ml::TrainData> SvmTrain::tdata() {
  ...
  std::vector<int> responses;
   ...
   for (auto f : train_file_list_) {
	...
	//auto  image = cv::imread("img.jpg");   //随便读取一张图片，查看如何提取特征并分类
    if (!image.data) {
    ...
    cv::Mat feature;
    extractFeature(image, feature);    //计算特征 在feature.cpp中getHistomPlusColoFeatures
    feature = feature.reshape(1, 1);

    samples.push_back(feature);           //将特征值返回至sample中
    responses.push_back(int(f.label)); //将label返回至response中
  }
