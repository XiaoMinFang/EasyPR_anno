...
int test_chars_identify() {
  ...
  cv::Mat plate = cv::imread("resources/image/1.jpg");
  ...
  int result = cs.charsSegment(plate, matChars);    //charsSegment字符分割，matChars.size()为字符的个数;
  if (result == 0) {
    for (size_t block_i = 0; block_i < matChars.size(); block_i++) {
      auto block = matChars[block_i];
      if (block_i == 0) {  //第一个字符一般都是汉字，所以单独出来     
        auto character = CharsIdentify::instance()->identify(block, true);                       //判断字符具体是个啥。。。。。。是什么算法？？？？
